...
| 강의 | 자료 |
|------|------|
| 내 노트북에서도 LLM이 돌아갈까? | 🖥️ [**웹 슬라이드 (HTML)**](local-llm/) · 📊 [PPT 다운로드](LLM_로컬실행_학습정리.pptx) · 📝 [요약 보기](#-내-노트북에서도-llm이-돌아갈까) |

> 🖥️ **웹 슬라이드**는 `local-llm/index.html` 을 브라우저로 열면 됩니다.
> `←` `→` 또는 화면 좌/우 클릭으로 이동 · `S` 학습노트 · `F` 전체화면 · 🌗 라이트/다크 테마 · 라이트/다크 자동 전환.
